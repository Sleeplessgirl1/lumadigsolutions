...
const ServiceCard = ({ service, style, isActive }: { service: typeof services[0]; style: typeof sectionStyles[0]; isActive: boolean }) => {
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-3 sm:inset-0 ${style.bg} rounded-3xl overflow-hidden flex items-center ${isActive ? "pointer-events-auto" : "pointer-events-none"}`}
    >
      {/* Centered icon */}
      ...
      >
        <div className="max-w-2xl py-0 mb-4 md:mb-[43px]">
          <h2 className={`text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-display font-black uppercase tracking-tight ${style.textColor} mb-3 leading-[0.95] py-0 my-4 md:my-[44px]`}>
            {service.title}
          </h2>
